...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Sulfur Dioxide Pollution Data in Los Angeles, CA"),
  # sidebarPanel(
...
#  )
  ...
  mainPanel( 
    ...
    fluidRow(
      ...
      splitLayout(cellWidths = c("33%", "33%", "33%"), leafletOutput(outputId = "mymap"), sidebarPanel (
        checkboxInput("markers", "Carbon Monoxide", FALSE),
        checkboxInput("markers", "Nitrogen Dioxide", FALSE),
        checkboxInput("markers", "Ozone", FALSE),
        checkboxInput("markers", "Sulfur Dioxide", TRUE)
...
server <- function(input, output) {#define the color pallate for the magnitidue of the earthquake
  ...
  output$mymap <- renderLeaflet({
    ...
    addCircles(data = dat,  lat = ~ longitude, lng = ~ latitude, weight = 1, radius = ~sqrt(test)*50, popup = ~as.character(AOD3), label = ~as.character(paste0("Magnitude: ", sep = " ", AOD3)),  fillOpacity = 0.5)
  ...
  output$phonePlot <- renderPlot ({
    # Render a barplot
    barplot(sulfurDioxideData,names.arg=year,xlab="Local Time",ylab="pollution (ppm)",col="blue", 
                                 main="Sulfur Dioxide Pollution in LA",border="red")
